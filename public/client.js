...
function loginSubmit() {
  $('#login-submit').on('click', function(event) {
    ...
    if (email == '') {
      $('#login-email-error').html(`<p>Please input email </p>`);
      $(removeLoginError);
      //            return false; // not sure about this line
    } else if (password == '') {
      $('#login-password-error').html(`<p>Please input password </p>`);
      $(removeLoginError);
    } else {
      $('#login-email').val('');
      $('#login-password').val('');
...
function signUpSubmit() {
  $('#sign-up-submit').on('click', function(event) {
    ...
    if (pantry == null) {
      $('#sign-up-pantry-error').html(
        `<p>Please select an existing pantry or create a new one </p>`
      );
      $(removeSignUpError);
      return;
    } else if (email === '') {
      ...
      .done(function(users) {
        ...
        if (password.length < 10) {
          $('#sign-up-password-error').html(
            `<p>Please enter password of at least 10 characters </p>`
          );
          $(removeSignUpError);
          errors = true;
        }
        if (password !== confirmPassword) {
          $('#sign-up-confirm-password-error').html(
          ...
          $.ajax({
            ...
            .done(function(result) {
              ...
              if (pantry != 'create') {
                ...
                alert(
                  'Thank you for signing up! Now please create a new Pantry to keep track of your food items.'
                );
                showNewPantryPage();
              }
            })
...
function displayItem(item) {
  let username =
    $('#loggedInUserFirstName').val() + ' ' + $('#loggedInUserLastName').val();
  let itemId = item._id;
...
function saveChanges() {
  $('.save-changes').on('click', function() {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      let description = row.getElementsByClassName('item-description')[0]
        .textContent;
...
function deleteItem() {
  $('.delete-item').on('click', function() {
    ...
    let numberDeletions = $("#pantryContent input[class='checkbox']:checked")
      .length;
...
function confirmDelete() {
  $('.confirmDelete').on('click', function() {
    ...
    if (numberDeletions < 1) {
      ...
      let confirmDeletion = confirm(
        'Are you sure you want to delete these ' + numberDeletions + ' items?'
      );
      if (confirmDeletion == true) {
        handleDeleteItem();
      } else {
...
function handleDeleteItem() {
  let rowsToDelete = $('#pantryContent')
    .find('input:checked')
    .parent()
    .parent();
